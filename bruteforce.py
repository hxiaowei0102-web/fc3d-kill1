# -*- coding: utf-8 -*-
"""
福彩3D 百十个杀一码 — 暴力穷举（单窗口：200期，三位置独立，v3 大池）
=============================================
...
并列裁决：命中率 → 公式更短 → 字典序。
【2026-09-04】已移除 300 期副窗口（老板决定只保留 200 期）。

产物：
  best_formula.json       最近 200 期窗口（预测跟踪沿用）
"""
import json
import numpy as np
...
def run_multi(verbose=True):
    """200期窗口穷举 → best_formula.json。
    返回 (results_200, None)；数据不足则 r200=None 且不写文件。"""
