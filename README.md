...
暴力穷举 · 近200期单窗口 · 每位置各杀1码 · GitHub Pages 云端全自动更新
...
2. **单窗口暴力穷举**：最近 200 期跑一遍（59特征 × 单/双/三特征线性组合 ≈ **905万条公式**，numpy向量化约 6 秒/窗口）→ 为百/十/个各选1条最优杀码公式
3. 固定公式回看 200 期 → 生成逐期真实预测回测表 → 部署到 GitHub Pages
...
- 跟踪基准 = **200期公式**（唯一窗口，删除300期副窗口后不重复跟踪）
...
| 文件 | 职责 |
|---|---|
| auto_update.py | 云端全自动更新入口（200期穷举+页面+每日预测跟踪） |
| track_predictions.py | 每日预测跟踪：验证+追加+累计统计 |
| predictions_log.csv | 预测跟踪日志（每日自动追加/验证） |
| fetch.py | 多源降级抓取 |
| engine.py / formulas.py | 数据引擎 / 公式库（59特征） |
| bruteforce.py / backtest.py | 暴力穷举(numpy) / N期回测 |
| gen_site.py | 生成网页（200期窗口数据） |
| best_formula.json | 200期 最优公式 |
...
## 本地运行
需要 Python 3.11+ 与 `pip install "numpy>=1.26,<3"`，然后 `python auto_update.py`（云端入口）或 `python update.py`（本地预览入口）
