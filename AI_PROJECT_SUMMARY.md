...
每晚定时抓最新开奖 → 用**最近 200 期**暴力穷举约 **905 万条线性公式** → 给「百位 / 十位 / 个位」各挑 1 条最优杀码公式 → 固定公式做 200 期逐期回测 → 生成手机端网页 → 部署到 GitHub Pages，并对每天的预测做**真实留痕 + 次日自动验证**。
...
| **窗口** | 只取**最近 200 期**历史来挑选公式（经 analyze_window 样本外验证后定为 200） |
...
| `bruteforce.py` | 暴力穷举：numpy 向量化扫 905 万公式、200 期窗口、流式维护三位置最优，写 `best_formula.json` | 共用 |
| `backtest.py` | 回测引擎：固定公式逐期回看 200 期（近期→远期），`predict_next()` 预测下期 | 共用 |
| `fetch.py` | 多源降级抓取 + 校验 + 追加 CSV（防倒灌、保序） | 共用 |
| `gen_site.py` | 生成手机端静态网页 `static/index.html`（含看板、回测表、跟踪表） | 共用 |
| `track_predictions.py` | **每日预测跟踪**：验证 pending + 追加今日预测 + 累计统计（写 `predictions_log.csv`） | 共用 |
...
| `best_formula.json` | 当前最优公式 + 命中率 + 数据范围 + 公式池大小（下游模块都读它） |
| `predictions_log.csv` | 预测跟踪日志（507 行 = 500 回填 + 5 真实 live + 1 pending） |
...
```python
WINDOW = 200
search_best(hh, tt, oo, window=200) -> (best, pool_size)
    # best = {'h':(公式名,命中率,命中数), 't':..., 'o':...}
    # 原理：最近200期 → 特征矩阵 F(200×59) → 逐条规格算 out=(F·系数+const)%10
    #      → out != 实际开奖 计数 = 命中数
    # 并列裁决：命中率 → 公式名更短 → 字典序
    # 数据 < 201 期时抛 ValueError
main()  # 计算并写 best_formula.json
```
> 905 万条 × 3 位置向量化约 2 分钟（云端实测）。
...
### track_predictions.py（真实预测留痕）
```python
backfill_history(combo, n=500)  # 首次启用：固定公式回填最近500期(源=backfill)
verify_pending(combo)           # pending 预测对应期已开奖 → 回填判定 hit/miss/partial
add_prediction(combo, issue=None)  # 追加今日新预测(源=live)，按 issue 幂等 upsert
summarize(combo=None) -> dict   # 累计统计：all_hit_rate/h_rate/t_rate/o_rate/
                                #   max_miss_streak/recent30_all/live/backfill 分源统计
main() -> track_changed:bool    # 三步：初始化回填→验证pending→追加今日
```
...
build_data() -> 大 dict：数据范围/下期期号/上期开奖/公式+白话解释/
    近200回测(百/十/个/全中/连错)/预测跟踪看板+近30期明细(近期→远期)/下次预测杀码
```
...
[2/6] bruteforce.search_best() 最近200期穷举905万 → 三位置各1条最优公式
      对比旧 best_formula.json：added==0 且公式未变 → 跳过页面生成(零无效更新)
[3/6] 有变化：写 best_formula.json → gen_site.main() 生成 static/index.html
[4/6] track_predictions.main() 每日预测跟踪
      ★必须先于[3]后执行(依赖新公式)：验证昨日 pending → 追加今日新预测
...
python auto_update.py     # 云端完整流程（含每日预测跟踪），本地可跑通全链路
python bruteforce.py      # 只重算 best_formula.json（约2分钟/次）
python gen_site.py        # 只用现有 best_formula.json 重新生成页面
...
### 7.1 设计哲学
- **回测 99% ≠ 真实水平**：那是"在最近200期里挑最准公式"的历史拟合，样本外必然回落。
...
- 3杀全中 80.8% 只比基线高约 8pp —— 其中相当部分来自 500 期回填（含公式拟合窗口内）。
- 真实 live 仅 5 期、80.0%，**样本太小不足以下结论**；项目的价值在于每天积累真实样本外数据。
...
2. **200 期窗口** 经样本外 walk-forward 验证为最优（analyze_window.py：N ∈ {100..350} 对比），
   但任何固定窗口都会随数据滚动换公式——**公式会每天/隔天变化是正常现象**，不是 bug。
...
| 改回测窗口 | `bruteforce.py WINDOW` + `best_formula.json` 的 window 字段 | 先跑 `analyze_window.py N` 做样本外对比再定 |
| 改网页样式/文案 | `gen_site.py` 的 `HTML_TEMPLATE` / `FEAT_ZH` | 跑 `gen_site.py` 后刷新页面 |
| 调整跟踪回填期数 | `track_predictions.py` main() 里 n=500 | 删 `predictions_log.csv` 重跑 main()（会重建） |
...
auto_update.py
   ...
   ├─[2] bruteforce: 最近200期 × 905万公式 numpy 穷举 → 百/十/个各1条最优
   ├─[3] 写 best_formula.json → gen_site → static/index.html（无变化则跳过）
   ├─[4] track: 验证昨日 pending + 落盘今日预测 → predictions_log.csv
   ├─[5] 跟踪有变而页面没生成 → 补生成
   └─[6] 结束
   │
   ▼
...
GitHub Pages → https://hxiaowei0102-web.github.io/fc3d-kill1/
   │
   ▼
手机浏览器查看：杀码卡片 / 200期回测表 / 每日跟踪看板（真实样本外）
```

---

*本文档由 AI 依据仓库代码（commit be045cd）与实时数据自动整理，供研究与二次开发参考。*
*仅供研究参考，不构成投注建议。*
